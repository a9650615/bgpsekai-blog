...
從舊 Ghost 站搬過來的小工具:把巴哈動漫瘋(`ani.gamer.com.tw`)的影片
彈幕抓下來、轉成 Aegisub 相容的 `.ass` 外掛字幕,再用 PotPlayer / mpv
等播放器掛載,就能在離線檔上重現原站彈幕。原本是 Ghost 頁面內嵌的
`<script>`,遷站時改寫成純 TypeScript module + Astro hydration UI,
並加上 40 個 `node:test` unit test 把 byte 級輸出鎖住。

- 路由:`/baha-danmu-to-ass/`(`src/pages/baha-danmu-to-ass.astro`)
- Library:`src/lib/baha-danmu/`(模組分工、ASS template 來源、
  upstream API 限制、與舊版 byte-diff 的相容細節都寫在
  [`src/lib/baha-danmu/README.md`](src/lib/baha-danmu/README.md))
- 測試:`npm test`
